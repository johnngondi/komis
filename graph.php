...
if ($conn) {
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		$message = "Graph for most Ailed Disease ".DateTime::createFromFormat('!m',$month)->format('F').", ".$year ;
		...
		//get all diseases
		$sql1 = "SELECT diseaseid,diseasename,diseasecat FROM disease ORDER BY diseasename ASC";
		$diseases = fetchdata($conn,$sql1);

		//use loop to get the details from prescription table

	}else{
		$message = "Graph for most Ailed Disease Year ".date("Y");
		$year = date('Y');
