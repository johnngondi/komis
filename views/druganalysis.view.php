	...
	<div class="col-md-1"></div>
		  <div class="col-md-10 col-sm-12 col-xsm-12 ">
				<div class="panel panel-default">
			        <div class="panel-heading"><h3 style="color:#009933;">Drug Analysis</h3></div>
			        <div class="panel-body">
			        <div class="panel-body">
			        	<h5 class="centering hidden-print"><b>Filter Using</b></h5>
			        	<form method="POST" class="hidden-print" action="druganalysis.php" name="analysisform" role="form">
						...
						<table class="table table-striped table-bordered table-hover ">
			              <thead class="table-contents">
			                <tr>
			                  <th class="centering">Drug Category</th>
			                  <th class="centering">Drug Name</th>
			                  <th class="centering">Usage (Doses)</th>
			                </tr>
			              </thead>
			              <tbody style="color:#009933;">
			              <?php 
			              for ($i=0; $i < $totaldrugs; $i++) { 
								$drug = $drugs[$i];
								$drugid = $drug['drugid'];
								$drugname = $drug['drugname'];
								$drugcat = $drug['drugcat'];

								//getdrug details
								$sql = "SELECT count(*) FROM medication WHERE drugid=$drugid".$criteria;
								$usage = getnumberofrows($conn, $sql);

								echo "
								<tr>
				                  <td class=\"centering\">".ucwords($drugcat)."</td>
				                  <td class=\"centering\">".ucwords($drugname)."</td>
				                  <td class=\"centering\">$usage</td>
				                </tr>
